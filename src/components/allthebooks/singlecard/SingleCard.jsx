...
const SingleCard = ({ img, title, bookId, setIdBook}) => {

  const {isDark} = useContext(ThemeContext)

  return (
    <div 
      className={`${isDark ? 'bg-dark text-bg-dark ' : ''} border-0 card h-100  shadow-sm} flex-row g-0 rounded-4 `}
    >
      <div className="row">
        ...
        <div className="col-8 ps-0 pe-2">
          <div className="h-100 card-body p-2 d-flex flex-column  justify-content-between">
            ...
            <div className={`card-action d-flex justify-content-end border-top pe-1 gap-3 ${isDark && 'border-secondary '} `}> 
              ...
              <LikeButton idBook={bookId} setIdBook={setIdBook}/>
            </div>
          </div>
        </div>
      </div>
